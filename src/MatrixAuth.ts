...
export class MatrixAuth {
  ...
  public async passwordRegister(
    ...
    try {
      response = await this.createTemplateClient().doRequest(
        "POST",
      ...
      if (e.statusCode === 401) {
        ...
        if (!hasFlow)
          throw new Error(
            "Failed to find appropriate login flow in User-Interactive Authentication"
          );
   ...
   * @param {string} deviceId The ID of the newly created device. Optional.
   * @returns {Promise<MatrixClient>} Resolves to a logged-in MatrixClient
   */
  public async passwordLogin(
    ...
    const response = await this.createTemplateClient().doRequest(
      "POST",
    ...
    if (
      response["well_known"] &&
      response["well_known"]["m.homeserver"] &&
